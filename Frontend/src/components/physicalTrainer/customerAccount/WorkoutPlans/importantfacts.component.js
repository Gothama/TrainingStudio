...
const siAPI1 = axios.create({
    baseURL: `${process.env.REACT_APP_BACKEND_URL}workoutplan/getallpaymentdetails/`
})

const siAPI2 = axios.create({
    baseURL: `${process.env.REACT_APP_BACKEND_URL}workoutplan/addpaymentdetails/`
})


export default class ImportantFactsWorkout extends Component {
    ...
    getdata() {
        siAPI1.post('/', { workoutplanID: this.props.id },
            {
    ...
    updateData=(event)=>{
        console.log(this.state.paid)
        siAPI2.post('/', { workoutplanID: this.props.id , paid:this.state.paid , amount:this.state.amount},
        {
            headers: { Authorization: "Bearer " + localStorage.getItem("token") }
        }).then(res => {
            console.log(res.data)
